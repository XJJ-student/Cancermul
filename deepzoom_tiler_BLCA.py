...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg'):
    ...
    n_levels = len(glob.glob('WSI_2_temp_files/*'))
    bag_path = os.path.join(out_base, img_class, img_name)
    os.makedirs(bag_path, exist_ok=True)
    if len(level)==1:
        patches = glob.glob(os.path.join('WSI_2_temp_files', '*', '*.'+ext))
        ...
        levels = [int(os.path.basename(i)) for i in glob.glob(os.path.join('WSI_2_temp_files', '*'))]
        levels.sort()
        low_patches = glob.glob(os.path.join('WSI_2_temp_files', str(levels[0]), '*.'+ext))
        for i, low_patch in enumerate(low_patches):
            ...
            for x_pos in high_x_list:
                for y_pos in high_y_list:
                    high_patch = glob.glob(os.path.join('WSI_2_temp_files', str(levels[1]), '{}_{}.'.format(x_pos, y_pos)+ext))
                    if len(high_patch)!=0:
...
if __name__ == '__main__':
    ...
    # cying
    parser.add_argument('-d', '--dataset', type=str, default='TCGA-lung', help='Dataset path')
    ...
    ####################################################################
    path_1 = './TCGA_Slide/TCGA-BLCA/'
    path_2 = './TCGA-BLCA/Region/mydisk/'

    all_slide_name = os.listdir(path_1)
    exist_slide_name = os.listdir(path_2)

    print(len(all_slide_name))
    print(len(all_slide_name))
    print(len(exist_slide_name))

    exist_slide_name_2 = []
    for i in all_slide_name:
        for j in exist_slide_name:
            if i.startswith(j):
                exist_slide_name_2.append(i)

    exist_slide_name_2 = list(set(exist_slide_name_2))
    print(len(exist_slide_name_2))

    all_slide_name = list(set(all_slide_name)-set(exist_slide_name_2))
    print(len(all_slide_name))
    print(all_slide_name[0])
    all_slides = [path_base+i for i in all_slide_name]
    print(len(all_slides))

    # pos-i_pos-j -> x, y
    for idx, c_slide in enumerate(all_slides):
        print('Process slide {}/{}'.format(idx+1, len(all_slides)))
        DeepZoomStaticTiler(c_slide, 'WSI_2_temp', levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
        nested_patches(c_slide, out_base, levels, ext=args.format)
        shutil.rmtree('WSI_2_temp_files') 
...
# nohup python deepzoom_tiler_BLCA.py -d ./TCGA_Slide/TCGA-BLCA/ -s 4096 -m 0 -b 20 > deepzoom_tiler_BLCA.txt 2>&1 &